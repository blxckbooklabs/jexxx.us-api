...
export const observabilityRoutes = async (server) => {
    /**
    ...
    server.get('/trace/:id/score', async (request, reply) => {
        ...
        try {
            ...
            const relevantScore = trace.scores?.find((s) => priorityScores.includes(s.name.toLowerCase()));
            return {
                traceId: id,
            ...
            server.log.error(`[Obs Proxy] Failed to fetch trace ${id}:`, error);
    ...
    server.get('/metrics/routes', async (request, reply) => {
        try {
            ...
            server.log.error('[Obs Metrics] Routes metrics error:', error);
    ...
    server.get('/metrics/pipeline', async (request, reply) => {
        try {
            ...
            const ingestSuccessRate = Math.round((stages.ingestion.successCount / (stages.ingestion.successCount + stages.ingestion.errorCount)) * 100);
            const vaultSuccessRate = Math.round((stages.vault.successCount / (stages.vault.successCount + stages.vault.errorCount)) * 100);
            // Format for diagram consumption
            const diagramData = {
            ...
            server.log.error('[Obs Metrics] Pipeline metrics error:', error);
    ...
    server.get('/trace/:id/timeline', async (request, reply) => {
        ...
        try {
            ...
            server.log.error(`[Obs Timeline] Failed to fetch timeline for ${id}:`, error);
