import { z } from "zod";
import { loadAccountQueryModule, loadVaultSessionHelpers, } from "../../lib/account-session.js";
...
export function getAccountSchemaPayload() {
    return {
        ...
        description: "Live, RLS-scoped account data — same sources as BLXCKCHAT Kingdom Agent, JEXXXUS CLI account_query, and BLXCKBOOK/NXT dashboard exports. Replaces stale manual JSON uploads for agents.",
        endpoints: {
...
export const accountRoutes = async (fastify) => {
    ...
    fastify.get("/summary", async (request, reply) => {
        ...
        if (!session)
            return;
        const asUserId = typeof request.query?.asUserId === "string" ? request.query.asUserId : undefined;
    ...
    fastify.get("/export", async (request, reply) => {
        ...
        const bbMod = await loadCliModule("lib/account-data/blxckbook-export.js");
        const nxtMod = await loadCliModule("lib/account-data/nxt-export.js");
        const tvMod = await loadCliModule("lib/account-data/tv-playlists.js");
        const payload = {
        ...
        if (target === "blxckbook" || target === "all") {
            payload.blxckbook = await bbMod.fetchBlxckbookExport(bbVault.client, bbVault.effectiveUserId, session.creds.email);
        }
