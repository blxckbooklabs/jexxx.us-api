...
import { aeoDiscovery, bibleSiteOrigin, fetchAeoSurfaces, fetchDailyMannaFromFeed, getChapterFromCorpus, getVerseFromCorpus, listCanons, loadCorpusIndex, publicChapterUrl, } from "../../lib/bible-corpus.js";
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const BIBLE_API_BASE = "https://bible-api.com";
const SOVEREIGN_VAULT_PATH = process.env.BIBLE_VAULT_PATH ||
...
const findSovereignFiles = (bookSlug, chapter) => {
    ...
    for (const vDir of vaultDirs) {
        ...
        const bookDir = books.find((b) => b.toLowerCase().replace(/[^a-z0-9]/g, "").includes(needle));
        if (!bookDir)
            continue;
...
async function chapterFromVault(book, chapterNum) {
    const sovereignFiles = findSovereignFiles(book, chapterNum);
    if (!sovereignFiles?.length)
        return null;
...
async function chapterFromBibleApi(book, chapterNum, translation) {
    const response = await fetch(`${BIBLE_API_BASE}/${encodeURIComponent(book)}+${chapterNum}?translation=${encodeURIComponent(translation)}`, { signal: AbortSignal.timeout(15_000) });
    if (!response.ok)
        return null;
...
export const bibleRoutes = async (server) => {
    ...
    server.get("/books", async (request) => {
        ...
        const data = (canonFilter
            ? books.filter((b) => b.canon.toLowerCase() === canonFilter.toLowerCase())
            : books).map((b) => ({
            name: b.name,
     ...
     */
    server.get("/manna", async (_req, reply) => {
     ...
     */
    server.get("/feed", async (_req, reply) => {
        try {
    ...
    server.get("/resolve", async (request, reply) => {
        ...
        const m = ref.match(/^((?:\d+\s+)?[A-Za-z][A-Za-z0-9\s.'-]*?)\s+(\d+)\s*[: ]\s*(\d+)\s*$/);
        if (!m?.[1] || !m[2] || !m[3]) {
            return reply.status(400).send({
                success: false,
    ...
    server.get("/search", async (request, reply) => {
        ...
        const ref = q.match(/^((?:\d+\s+)?[A-Za-z][A-Za-z0-9\s.'-]*?)\s+(\d+)\s*[: ]\s*(\d+)\s*$/);
        if (ref?.[1] && ref[2] && ref[3]) {
            const hit = await getVerseFromCorpus(ref[1].trim(), parseInt(ref[2], 10), parseInt(ref[3], 10));
            if (!hit) {
    ...
    server.get("/:book/:chapter/:verse", async (request, reply) => {
        ...
        // bible-api.com
        try {
            const response = await fetch(`${BIBLE_API_BASE}/${encodeURIComponent(book)}+${chapterNum}:${verseNum}?translation=${encodeURIComponent(translation)}`, { signal: AbortSignal.timeout(15_000) });
            if (response.ok) {
                const data = (await response.json());
                return {
                    success: true,
