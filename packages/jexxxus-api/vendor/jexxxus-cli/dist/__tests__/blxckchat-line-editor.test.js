...
import { applyLineEditorAction, createLineEditorState, deleteWordBackward, getSelectionRange, insertText, killLine, killToEnd, killToStart, moveWordLeft, moveWordRight, isSpuriousTerminalInput, resolveInsertChar, resolveLineEditorKey, selectAll, selectWordAt, wordBoundsAt, wordLeft, wordRight, } from "../lib/blxckchat/ui/editor/line-editor.js";
...
test("ctrl+shift+a selects from cursor to line start", () => {
    let state = createLineEditorState("hello world");
    state = { ...state, cursor: 8 };
    state = applyLineEditorAction(state, resolveLineEditorKey({ name: "a", ctrl: true, shift: true }));
...
test("ctrl+e moves to line end with optional shift extend", () => {
    ...
    state = applyLineEditorAction(state, resolveLineEditorKey({ name: "e", ctrl: true, shift: true }));
...
test("ctrl+shift+left selects from cursor to line start", () => {
    ...
    state = applyLineEditorAction(state, resolveLineEditorKey({ name: "left", ctrl: true, shift: true }));
    assert.deepEqual(getSelectionRange(state), { start: 0, end: 7 });
});
test("ctrl+shift+right selects from cursor to line end", () => {
    let state = createLineEditorState("one two three");
    state = { ...state, cursor: 7 };
    state = applyLineEditorAction(state, resolveLineEditorKey({ name: "right", ctrl: true, shift: true }));
...
//# sourceMappingURL=blxckchat-line-editor.test.js.map