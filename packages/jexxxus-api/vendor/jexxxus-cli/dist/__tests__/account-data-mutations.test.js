...
import { addContact, updateContact, deleteContact, addJournalEntry, updateJournalEntry, deleteJournalEntry, managePlaylist, addContactEvent, updateContactEvent, deleteContactEvent, } from "../lib/account-data/mutations.js";
/**
 ...
 */
function createMockVaultClient(initialTables) {
    ...
    function from(table) {
        ...
        const filters = [];
        const applyFilters = (candidates) => candidates.filter((r) => filters.every((f) => r[f.col] === f.val));
        const chain = {
            select() {
                return chain;
            },
            ...
            then(resolve) {
                if (mode === "update") {
                    rows = rows.map((r) => applyFilters([r]).length > 0 ? { ...r, ...(updatePayload ?? {}) } : r);
                    tables.set(table, rows);
...
function fakeSession(clients) {
    return {
        creds: { userId: "user_test", email: "test@example.com" },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        blxckbook: (clients.blxckbook ?? createMockVaultClient({})),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        nxt: (clients.nxt ?? createMockVaultClient({})),
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        tv: (clients.tv ?? createMockVaultClient({})),
        isSuperAdmin: false,
    };
}
...
//# sourceMappingURL=account-data-mutations.test.js.map