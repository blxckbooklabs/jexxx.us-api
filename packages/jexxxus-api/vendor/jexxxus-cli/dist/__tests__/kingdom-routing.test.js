...
import { isVaultPrimaryPrompt, planAccountTools, } from "../lib/blxckchat/account-routing.js";
import { isKingdomSurfaceName, isKingdomSurfacePrompt, } from "../lib/blxckchat/kingdom-surfaces.js";
import { COMPANION_VERSE_SETS, formatKingdomRoutingHint, planKingdomTools, } from "../lib/blxckchat/kingdom-routing.js";
function expectTools(prompt, opts) {
    const plan = planKingdomTools(prompt);
    for (const tool of opts.include) {
        assert.ok(plan.tools.includes(tool), `Expected ${tool} in [${plan.tools.join(", ")}] for: ${prompt}`);
    }
    for (const tool of opts.exclude ?? []) {
        assert.ok(!plan.tools.includes(tool), `Expected ${tool} absent from [${plan.tools.join(", ")}] for: ${prompt}`);
    }
    for (const hint of opts.slashHints ?? []) {
        assert.ok(plan.slashHints.some((h) => h.includes(hint)), `Expected slash hint containing "${hint}" for: ${prompt}`);
    }
    for (const verse of opts.companionVerses ?? []) {
        assert.ok(plan.companionVerses.includes(verse), `Expected companion verse "${verse}" in [${plan.companionVerses.join(", ")}] for: ${prompt}`);
    }
}
...
test("regression 10: short follow-up routes via conversation context (Proverbs 31)", () => {
    const plan = planKingdomTools("keep going", {
        conversationContext: "Lil' Bible: the blonde with the Proverbs 31 bookmark. Hannah: read the draft on the way to her car.",
    });
...
//# sourceMappingURL=kingdom-routing.test.js.map