...
test("fetchBlxckbookExport assembles export payload matching SettingsView schema", async () => {
    ...
    const result = await fetchBlxckbookExport(supabase, "user_1", "alice@example.com");
...
test("fetchBlxckbookExport throws with a descriptive message on query error", async () => {
    ...
    await assert.rejects(() => fetchBlxckbookExport(supabase, "user_1", "alice@example.com"), /Failed to fetch contacts: permission denied/);
});
test("fetchNxtExport returns raw vessels + contact_events rows scoped to the user", async () => {
    ...
    const result = await fetchNxtExport(supabase, "user_1");
    assert.equal(result.format_version, "1.0");
    assert.equal(result.contacts.length, 1);
...
test("fetchNxtExport throws with a descriptive message on query error", async () => {
    ...
    await assert.rejects(() => fetchNxtExport(supabase, "user_1"), /Failed to fetch vessels: relation does not exist/);
});
//# sourceMappingURL=account-data.test.js.map