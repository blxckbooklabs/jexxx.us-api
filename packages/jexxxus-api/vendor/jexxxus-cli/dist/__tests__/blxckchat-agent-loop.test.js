...
function makeSingleToolCallProvider() {
    let callCount = 0;
    return {
        id: "anthropic",
...
test("runAgent reports an error for an unrecognized tool name", async () => {
    const provider = {
        id: "anthropic",
        async chat(messages) {
            const alreadyToldItsUnknown = messages.some((m) => m.role === "tool" && m.content.includes("unknown tool"));
...
//# sourceMappingURL=blxckchat-agent-loop.test.js.map