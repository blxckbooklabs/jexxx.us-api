...
import { StreamBuffer, formatStreamingChunk, finalizeStreamedContent, streamTokens, } from "../lib/blxckchat/ui/renderer/streaming.js";
...
test("streamTokens invokes onUpdate for each chunk and finalizes", async () => {
    const updates = [];
    const raw = await streamTokens("ab", (partial) => updates.push(partial), 1, 0);
    assert.equal(raw, "ab");
    assert.ok(updates.length >= 2);
    const last = updates[updates.length - 1];
    assert.ok(last.includes("a") || last.includes("▌"));
});
//# sourceMappingURL=blxckchat-streaming.test.js.map