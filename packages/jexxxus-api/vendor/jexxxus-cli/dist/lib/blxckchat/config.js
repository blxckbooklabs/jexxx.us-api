...
function writeUnifiedFile(mutator) {
    ensureConfigDir();
...
function mergeLastUsedModel(config, lastUsed) {
    if (!config)
        return null;
 ...
 */
export function resolveStartupProvider(explicitName) {
    const file = readUnifiedFile();
...
export function saveLastUsedProvider(config) {
...
export function listProvidersRedacted() {
    const file = loadCredentials();
...
export async function runConfigureFlow() {
    ...
    if (entry.requiresBaseUrl || entry.baseUrl) {
        ...
        const urlAnswer = await prompt(`Base URL${defaultUrl ? ` (default: ${defaultUrl})` : ""}: `);
        baseUrl = urlAnswer || defaultUrl || undefined;
        if (entry.requiresBaseUrl && !baseUrl) {
            throw new Error("Base URL is required.");
        }
    }
    const modelAnswer = await prompt(`Model (default: ${defaultModelFor(entry)}): `);
    ...
    if (apiKey)
        connectInput.apiKey = apiKey;
    if (baseUrl)
        connectInput.baseUrl = baseUrl;
