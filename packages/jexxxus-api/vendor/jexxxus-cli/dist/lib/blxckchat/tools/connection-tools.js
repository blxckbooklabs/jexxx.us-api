...
import { listNotifications, connectContactBack, getRelationshipTier, } from "../../account-data/connections.js";
export const listNotificationsTool = {
    name: "list_notifications",
    description: "List the signed-in user's contact-connection notifications (someone added them on JEXXXUS) " +
            ...
            for (const n of contactNotifications) {
                const status = n.alreadyConnected
                    ...
                    : n.read
                        ? ""
                        : "[unread] ";
                lines.push(`  • ${status}${n.actor_name} added you (actor_user_id: ${n.actor_user_id}) — ${n.created_at}`);
            }
        }
        if (pendingInvites.length > 0) {
            lines.push("Pending event invites:");
            for (const inv of pendingInvites) {
                lines.push(`  • "${inv.title}" from ${inv.organizer_name} on ${inv.event_date}${inv.location ? ` at ${inv.location}` : ""} (invite id: ${inv.id})`);
            }
        }
        return lines.join("\n");
    },
};
export const connectContactBackTool = {
    name: "connect_contact_back",
    description: "Connect back with a Clerk user who added the signed-in user as a contact (from " +
...
export const getRelationshipStatusTool = {
    name: "get_relationship_status",
    description: "Get the signed-in user's current relationship tier and points total with a Clerk-linked " +
