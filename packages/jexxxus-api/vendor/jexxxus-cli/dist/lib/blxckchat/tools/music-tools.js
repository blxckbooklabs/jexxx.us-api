import { fetchMusicLlmsTxt, getMusicDocsSummary, getMusicEntry, getMusicPublicEndpoints, listMusicCatalog, MUSIC_CATALOG, searchMusicCatalog, } from "../../music.js";
import { formatMusicCatalogList, formatMusicDiscover, formatMusicEntryFull, formatMusicEntryMeta, } from "./music-format.js";
function resolveMusicAction(raw) {
    ...
    if (action === "meta")
        return "meta";
    if (action === "discover" || action === "feed" || action === "rss")
        return "discover";
    if (action === "docs" || action === "about")
        return "docs";
    return null;
}
/**
...
export const musicTool = {
    name: "music_query",
    description: "Query JEXXXUS Music — Crucifly Records on music.jexxx.us, gospel/Christian hip-hop beats, " +
    ...
    parameters: {
        ...
        properties: {
            ...
            slug: {
                type: "string",
                description: "Catalog slug from list/search (required for get/meta), e.g. beats, kits, song-of-dylan, traktrain-store",
            },
            query: {
                type: "string",
    ...
    async execute(args) {
        ...
        switch (action) {
            ...
            case "discover": {
                ...
                return formatMusicDiscover(endpoints, all.length, all.slice(0, 6), llms);
            }
            case "list": {
            ...
            case "meta": {
                ...
                if (!key) {
                    return ("Error: 'slug' or 'query' is required for meta. " +
                        "Run action=list or action=search first, then pass the catalog slug.");
                }
                const entry = getMusicEntry(key);
                if (!entry)
                    return `No JEXXXUS Music entry found matching "${key}".`;
                return formatMusicEntryMeta(entry, endpoints);
            }
            case "get": {
                const key = slug || query;
                if (!key) {
                    return ("Error: 'slug' or 'query' is required for get. " +
                        "Run action=list or action=search first, then pass the catalog slug.");
                }
                const entry = getMusicEntry(key);
                if (!entry)
...
//# sourceMappingURL=music-tools.js.map