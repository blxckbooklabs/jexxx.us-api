...
export function formatTvVideoList(videos, total, source) {
    if (videos.length === 0) {
    ...
    for (const [i, video] of videos.entries()) {
        ...
        if (video.description && video.description !== video.title) {
            const preview = video.description.length > 160
...
export function formatTvVideoMeta(meta, endpoints) {
    const lines = [
        meta.title,
        `URL: ${meta.url}`,
        `Slug: ${meta.slug}`,
    ...
    if (meta.uploadDate)
        lines.push(`Published: ${formatDate(meta.uploadDate)}`);
    if (meta.channel)
    ...
    lines.push("", "Public SEO / discovery:", `RSS: ${endpoints.feed}`, `Sitemap: ${endpoints.sitemap}`, `Video sitemap: ${endpoints.sitemapVideo}`, `llms.txt: ${endpoints.llms}`, `llms-full.txt: ${endpoints.llmsFull}`);
    return lines.join("\n");
}
export function formatTvDiscover(endpoints, videoCount, samples, categories, source) {
    const lines = ["JEXXXUS | TV public endpoints:"];
    if (source) {
        lines.push(`Source: ${source.detail}`);
    }
    lines.push(`Site: ${endpoints.site}`, `Playlists: ${endpoints.playlists}`, `Subscription: ${endpoints.subscription}`, `RSS: ${endpoints.feed}`, `Sitemap: ${endpoints.sitemap}`, `Video sitemap: ${endpoints.sitemapVideo}`, `robots.txt: ${endpoints.robots}`, `llms.txt: ${endpoints.llms}`, `llms-full.txt: ${endpoints.llmsFull}`, "", `Published videos: ${videoCount}`);
...
export function formatTvVideoFull(video) {
    const header = [
        video.title,
        `URL: ${video.url}`,
...
//# sourceMappingURL=tv-format.js.map