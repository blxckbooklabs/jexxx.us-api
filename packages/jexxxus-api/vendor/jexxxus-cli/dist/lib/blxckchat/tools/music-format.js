export function formatMusicCatalogList(entries, total) {
    if (entries.length === 0) {
    ...
    lines.push("Beats and kits are licensed via Traktrain — use action=get with slug for detail, or action=discover for llms.txt.");
    return lines.join("\n").trimEnd();
}
export function formatMusicEntryFull(entry) {
    const lines = [
        entry.title,
...
export function formatMusicEntryMeta(entry, endpoints) {
    const lines = [
        entry.title,
        `URL: ${entry.url}`,
        `Slug: ${entry.slug}`,
...
export function formatMusicDiscover(endpoints, catalogCount, samples, llmsExcerpt) {
    const lines = [
    ...
    if (llmsExcerpt) {
        const excerpt = llmsExcerpt.length > 2_000
...
//# sourceMappingURL=music-format.js.map