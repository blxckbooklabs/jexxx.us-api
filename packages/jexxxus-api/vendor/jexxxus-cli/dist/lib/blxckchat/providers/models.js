import { getCatalogEntry, listCatalogEntries, normalizeProviderId, resolveBaseUrl, } from "./catalog.js";
...
export function supportsLiveModelDiscovery(entry) {
    if (entry.adapter === "ollama")
        return true;
    if (entry.adapter !== "openai")
        return false;
    if (entry.id === "azure-openai")
        return false;
    if (entry.requiresBaseUrl)
        return true;
    return Boolean(entry.baseUrl) || entry.id === "openai";
}
function resolveDiscoveryBaseUrl(entry, override) {
...
function mergeModelIds(staticModels, liveModels, catalogId) {
...
export async function listModelsForProvider(catalogId, opts) {
...
export async function listModelOptions(activeConfig) {
    const seen = new Set();
    const options = [];
    const push = (id, provider, source, label) => {
    ...
    if (activeConfig) {
        ...
        if (entry) {
            ...
            for (const id of modelIds) {
                ...
                const source = hasLive && !inStatic ? "live" : "suggested";
...
export function findModelMatch(query, options) {
    const q = query.trim().toLowerCase();
    if (!q)
        return null;
    const exact = options.find((o) => o.id.toLowerCase() === q ||
        o.label.toLowerCase() === q ||
        `${o.provider}/${o.id}`.toLowerCase() === q);
    if (exact)
        return exact;
    const partial = options.filter((o) => o.id.toLowerCase().includes(q) ||
...
export function cycleModelOption(options, current, direction) {
