...
export const SlashPopup = ({ suggestions, selectedIndex, visible, }) => {
    ...
    const shown = suggestions.slice(scrollOffset, scrollOffset + VISIBLE_SUGGESTIONS);
    const height = Math.min(shown.length + 3, MAX_HEIGHT);
    return (_jsxs(Box, { position: "absolute", bottom: 4, left: 1, width: "68%", height: height, borderStyle: "round", borderColor: PINK, backgroundColor: BG_ELEVATED, flexDirection: "column", children: [_jsx(Text, { color: PINK, children: " /commands " }), shown.map((s, i) => {
                const actualIndex = scrollOffset + i;
                const isSel = actualIndex === selectedIndex;
                const clean = sanitizeDesc(s.description);
                const desc = clean.length > DESCRIPTION_MAX
                    ? `${clean.slice(0, DESCRIPTION_MAX - 3)}...`
                    : clean;
                return (_jsxs(Box, { width: "100%", backgroundColor: isSel ? PINK : undefined, height: 1, children: [_jsxs(Text, { bold: isSel, color: isSel ? BG_ELEVATED : TEXT, children: [" ", s.label, " "] }), _jsx(Text, { color: isSel ? BG_ELEVATED : TEXT_MUTED, children: desc })] }, s.value));
            })] }));
};
//# sourceMappingURL=SlashPopup.js.map