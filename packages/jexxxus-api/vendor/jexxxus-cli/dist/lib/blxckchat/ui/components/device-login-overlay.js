...
import { getCredentialsDir, openDeviceAuthBrowser, pollDeviceAuth, startDeviceAuth, } from "../../../auth.js";
...
function writeDeviceCodeFallback(userCode) {
...
export function createDeviceLoginOverlay(screen, options = {}) {
    let visible = false;
    let cancelled = false;
    let dotTimer = null;
    let activeUserCode = "";
    let copyHint = "{gray-fg}Copying code to clipboard…{/}";
    ...
    const copyActiveCode = async (renderView, status) => {
        if (!activeUserCode)
            return;
    ...
    const handleKeypress = (_ch, key, renderView, status) => {
        if (!visible)
            return;
    ...
    return {
        ...
        async run() {
            ...
            const updateView = (status) => {
                pollStatus = status;
                render(formatDeviceLoginOverlayContent({
                    userCode,
                    verificationUrl,
                    expiresMinutes,
                    status,
...
//# sourceMappingURL=device-login-overlay.js.map