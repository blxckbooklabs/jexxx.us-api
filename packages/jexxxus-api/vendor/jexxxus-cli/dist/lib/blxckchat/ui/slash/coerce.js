...
function canonicalCommandName(token) {
    const cmd = BUILTIN_SLASH_COMMANDS.find((c) => c.name === token || (c.aliases ?? []).includes(token));
    return cmd?.name ?? token;
}
/** Expand partial slash commands (e.g. /mod → /model) before dispatch. */
export function coerceSlashLine(line) {
