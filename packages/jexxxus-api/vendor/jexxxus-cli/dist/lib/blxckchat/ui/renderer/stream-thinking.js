...
function partialOpenerHoldback(pending) {
    let hold = 0;
...
export function formatThinkingWaitState() {
    return `${TAG.dim}  ◇ thinking…${TAG.dimEnd}${BLESSED_STREAM_CURSOR}`;
}
function formatThinkingStreamBody(text, withCursor) {
 ...
 */
export function formatLiveStreamDisplay(state) {
    const parts = [];
    const showThinking = state.hasThinking || state.inThinking;
    if (showThinking) {
        parts.push(`${TAG.dim}  ${TAG.pink}[▼ think]${TAG.pinkEnd}${TAG.dimEnd}`);
        const thinkingCursor = state.inThinking && (!state.visible.trim() || state.thinking.length === 0);
...
//# sourceMappingURL=stream-thinking.js.map