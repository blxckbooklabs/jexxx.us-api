import { buildProviderConfig, getProviderByName, listProvidersRedacted, upsertProvider, } from "../../config.js";
import { defaultModelFor, getCatalogEntry, listCatalogEntries, resolveBaseUrl, resolveEnvApiKey, } from "../../providers/catalog.js";
import { listModelsForProvider, supportsLiveModelDiscovery, } from "../../providers/models.js";
import { resolveProvider } from "../../providers/registry.js";
import { createPickerOverlay } from "./picker-overlay.js";
import { createPromptOverlay } from "./prompt-overlay.js";
...
function unifiedPickerItems(activeName) {
    const items = [];
...
export function createProviderOverlay(screen, opts) {
    ...
    const isBusy = () => flowActive || picker.isVisible() || modelPicker.isVisible() || prompt.isVisible();
    const showPrompt = (options) => new Promise((resolve) => {
        setImmediate(() => {
            void prompt.ask(options).then(resolve);
        });
    });
    const runSetupFlow = async (entry) => {
        flowActive = true;
        let apiKey;
        let baseUrl;
        try {
            ...
            opts.onMessage(`Provider ready: "${built.name}" (${built.provider}/${built.model})`);
        }
        catch (err) {
    ...
    picker.setOnPick((item) => {
        ...
        if (item.id.startsWith("saved:")) {
            ...
            try {
                ...
                opts.onMessage(`Switched to "${resolved.name}" (${resolved.provider}/${resolved.model})`);
            }
            catch (err) {
...
//# sourceMappingURL=provider-overlay.js.map