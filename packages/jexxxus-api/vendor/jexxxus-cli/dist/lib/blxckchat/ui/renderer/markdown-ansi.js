 ...
 */
export function normalizeAgentMarkup(text) {
    return text
        .replace(/\{italic\}/gi, "_")
        .replace(/\{\/italic\}/gi, "_")
        .replace(/\{bold\}/gi, "**")
...
export function trimPartialClosingFences(tokens) {
    ...
    if (!marker || !lastLine || lastLine.length >= marker.length)
        return;
    if (lastLine !== marker[0]?.repeat(lastLine.length))
        return;
...
function renderToken(token) {
    switch (token.type) {
        case "text":
            return token.text;
        case "strong":
            return c.bold(renderInline(token.tokens));
        case "em":
            return c.italic(renderInline(token.tokens));
        case "codespan":
            return muted(token.text);
        case "link": {
            const t = token;
 ...
 */
export function markdownToAnsi(markdown) {
...
//# sourceMappingURL=markdown-ansi.js.map