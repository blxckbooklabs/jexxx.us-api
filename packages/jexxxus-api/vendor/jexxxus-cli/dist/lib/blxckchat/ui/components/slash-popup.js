...
export function stepListIndex(current, delta, total) {
    if (total <= 0)
        return 0;
...
export function createSlashPopup(screen) {
    let selectedIndex = 0;
    let visible = false;
    let onPickHandler;
    ...
    const formatItem = (s) => {
        const desc = s.description.length > 48 ? `${s.description.slice(0, 45)}…` : s.description;
    ...
    const wireListItemMouse = (item, idx) => {
        if (!mouseEnabled || item.__slashMouseWired)
            return;
...
export function applySuggestion(currentValue, suggestion, mode) {
    if (mode === "command") {
...
//# sourceMappingURL=slash-popup.js.map