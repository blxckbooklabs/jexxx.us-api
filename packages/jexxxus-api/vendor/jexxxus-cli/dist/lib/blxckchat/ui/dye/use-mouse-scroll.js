...
export function useMouseScroll(callbacks, enabled) {
    const { stdin, isRawModeSupported } = useStdin();
    const stateRef = useRef({ callbacks, enabled });
    stateRef.current = { callbacks, enabled };
    useEffect(() => {
        if (!isRawModeSupported || !isBlessedMouseEnabled())
            return;
        const handleData = (chunk) => {
            ...
            while ((match = MOUSE_SEQUENCE_RE.exec(text)) !== null) {
                ...
                if (parsed.action === "wheel-up") {
                    for (let i = 0; i < WHEEL_LINES; i++)
                        stateRef.current.callbacks.onScrollUp();
                }
                else if (parsed.action === "wheel-down") {
                    for (let i = 0; i < WHEEL_LINES; i++)
                        stateRef.current.callbacks.onScrollDown();
                }
            }
        };
...
//# sourceMappingURL=use-mouse-scroll.js.map