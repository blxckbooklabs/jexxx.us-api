...
const WORD_CHAR = /[\w./:@-]/;
export function createLineEditorState(text = "") {
...
export function wordLeft(text, pos) {
...
export function wordRight(text, pos) {
    let i = Math.max(0, Math.min(pos, text.length));
...
export function wordBoundsAt(text, index) {
...
export function selectWordAt(state, index) {
...
export function lineEditorViewScrollStart(state, viewWidth) {
...
export function charIndexFromMouseX(mouseX, state, viewWidth) {
...
function clampCursor(state, cursor) {
    const next = Math.max(0, Math.min(state.text.length, cursor));
    return { ...state, cursor: next };
}
function moveTo(state, cursor, extend) {
...
export function moveCharLeft(state, extend = false) {
    return moveTo(state, state.cursor - 1, extend);
}
export function moveCharRight(state, extend = false) {
    return moveTo(state, state.cursor + 1, extend);
}
export function moveWordLeft(state, extend = false) {
    return moveTo(state, wordLeft(state.text, state.cursor), extend);
}
export function moveWordRight(state, extend = false) {
    return moveTo(state, wordRight(state.text, state.cursor), extend);
}
export function moveLineStart(state, extend = false) {
    return moveTo(state, 0, extend);
}
export function moveLineEnd(state, extend = false) {
...
function withSelectionRemoved(state) {
...
export function insertText(state, ch) {
...
export function deleteWordBackward(state) {
    const range = getSelectionRange(state);
    if (range) {
        const { text, cursor } = deleteRange(state.text, range.start, range.end);
        return { text, cursor, selectionAnchor: null };
...
function parseKeyModifiers(key) {
    const full = key.full ?? "";
...
export function isSpuriousTerminalInput(ch, key) {
    const full = key.full ?? "";
    if (full.includes("\x1b[M") || full.includes("\x1b[<"))
        return true;
    if (ch.includes("\x1b[M") || ch.includes("\x1b[<"))
        return true;
    if (ch.length >= 6 && /^[\x1b\[MCGF@.,:/\-\d^]+$/i.test(ch))
        return true;
    return false;
}
/** Resolve a single printable character from a key event (incl. punctuation). */
export function resolveInsertChar(key) {
    const { name, shift, meta, ctrl } = parseKeyModifiers(key);
    if (meta || ctrl)
        return null;
    const ch = key.ch ?? "";
    if (isSpuriousTerminalInput(ch, key))
        return null;
...
export function resolveLineEditorKey(key) {
    const { name, shift, meta, ctrl } = parseKeyModifiers(key);
    if (isSpuriousTerminalInput(key.ch ?? "", key))
        return { type: "noop" };
    ...
    if (name === "left") {
        ...
        if (ctrl && shift)
            return { type: "move-line-start", extend: true };
        // ⇧⌥← — select word left.
        if (meta && shift)
            return { type: "move-word-left", extend: true };
        // ⌥← — word left (M-left).
        if (meta)
            return { type: "move-word-left", extend: false };
    ...
    if (name === "right") {
        if (ctrl && shift)
            return { type: "move-line-end", extend: true };
        if (meta && shift)
            return { type: "move-word-right", extend: true };
        if (meta)
            return { type: "move-word-right", extend: false };
        if (ctrl)
            return { type: "move-word-right", extend: false };
...
export function applyLineEditorAction(state, action) {
...
export function renderLineEditorView(state, viewWidth, selectionTagOrOptions = "{#ec4899-fg}{inverse}", selectionEndTag = "{/}{/}") {
    const options = typeof selectionTagOrOptions === "string"
