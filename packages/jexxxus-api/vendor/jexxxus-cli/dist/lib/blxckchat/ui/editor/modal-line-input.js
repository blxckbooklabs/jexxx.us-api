import { applyLineEditorAction, createLineEditorState, getSelectionRange, insertText, renderLineEditorView, resolveLineEditorKey, } from "./line-editor.js";
...
export function createModalLineInput(initial = "", options = {}) {
    let state = createLineEditorState(initial);
    const mask = Boolean(options.mask);
    const maskChar = options.maskChar ?? "•";
    const toLineKey = (ch, key) => {
        const lineKey = { ch };
        if (key.name)
            lineKey.name = key.name;
        if (key.shift)
            lineKey.shift = true;
        if (key.meta)
            lineKey.meta = true;
        if (key.ctrl)
            lineKey.ctrl = true;
        if (key.full)
            lineKey.full = key.full;
        return lineKey;
    };
    const resolve = (ch, key) => resolveLineEditorKey(toLineKey(ch, key));
    return {
        getText() {
            return state.text;
        },
...
export function insertModalLinePaste(handle, text) {
...
function renderMaskedView(state, viewWidth, maskChar) {
    const range = getSelectionRange(state);
    let start = 0;
    if (state.text.length > viewWidth) {
        start = Math.max(0, Math.min(state.cursor - Math.floor(viewWidth * 0.4), state.text.length - viewWidth));
    }
...
//# sourceMappingURL=modal-line-input.js.map