import { getProviderByName, loadCredentials, listProvidersRedacted, saveLastUsedProvider, upsertProvider, } from "../../config.js";
...
import { activateDivinityPersona, clearActiveDivinity, formatDivinityActivationMessage, formatDivinityClearedMessage, } from "../../divinities/session.js";
import { findDivinityPersona, listDivinityPersonas, } from "../../divinities/source.js";
import { formatSlashHelp, resolveSlashCommandName, } from "./registry.js";
import { deleteCredentials, formatAuthStatusLines, loadCredentials as loadAuthCredentials, promptYesNo, refreshAccessTokenViaServer, runInteractiveDeviceLogin, saveCredentials, } from "../../../auth.js";
...
function findProviderConfig(name) {
    const file = loadCredentials();
    return (file.providers.find((p) => p.name.toLowerCase() === name.toLowerCase()) ?? null);
}
...
async function handleModel(args, state) {
    ...
    if (!args) {
        ...
        lines.push(`Current model: ${state.activeConfig.provider}/${state.activeConfig.model}`, "", "Available models (use /model <name>):");
        for (const opt of options.slice(0, 20)) {
            const marker = opt.id === state.activeConfig.model &&
        ...
        if (options.length > 20) {
            lines.push(`  … and ${options.length - 20} more (type to filter with /model <term>)`);
        }
...
async function handleProvider(args, state) {
    ...
    lines.push(`Switched to "${resolved.name}" (${resolved.provider}/${resolved.model})`);
    return { handled: true, messages: lines };
}
function handleSession(state) {
...
async function handleDivinities(args, state) {
...
export async function dispatchSlashCommand(line, state) {
