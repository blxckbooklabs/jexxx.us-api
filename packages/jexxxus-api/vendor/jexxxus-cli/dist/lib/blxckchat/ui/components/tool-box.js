...
export function summarizeToolResultForDisplay(result, status, toolName) {
    if (status === "pending")
        return "running…";
    ...
    if ((toolName === "veil_query" || toolName === "tv_query") &&
        status === "success") {
        return trimmed;
    }
    const isVeilCatalog = toolName === "veil_query" &&
        (trimmed.startsWith("VEIL articles") ||
            trimmed.startsWith("VEIL public endpoints") ||
            trimmed.startsWith("No VEIL articles"));
    const isTvCatalog = toolName === "tv_query" &&
    ...
    const isVeilMeta = toolName === "veil_query" && trimmed.includes("Public SEO / discovery:");
    const isTvMeta = toolName === "tv_query" && trimmed.includes("Public SEO / discovery:");
    const isVeilArticleBody = toolName === "veil_query" && trimmed.includes("\n---\n");
    const isTvVideoBody = toolName === "tv_query" && trimmed.includes("\n---\n");
    ...
    const suffix = lines.length > TOOL_DISPLAY_MAX_LINES || trimmed.length > TOOL_DISPLAY_MAX_CHARS
        ? `\n… (${lines.length} lines, ${trimmed.length} chars total)`
        : "";
    const body = clipped.length > TOOL_DISPLAY_MAX_CHARS
        ? `${clipped.slice(0, TOOL_DISPLAY_MAX_CHARS)}…`
        : clipped;
    return `${body}${suffix}`;
}
export function formatToolLinePlain(toolName, result, status) {
    const icon = STATUS_ICONS[status];
    const label = status === "pending" ? "running…" : summarizeToolResultForDisplay(result, status, toolName);
    return `  ${icon} tool:${toolName} → ${label}\n`;
}
export function formatToolLine(toolName, result, status) {
    const icon = STATUS_ICONS[status];
...
export function formatToolResult(entry) {
    return formatToolLine(entry.toolName, entry.result, entry.status);
}
export function formatToolResults(entries) {
    return entries.map(formatToolResult).join("");
}
export function formatToolResultsPlain(entries) {
    return entries
