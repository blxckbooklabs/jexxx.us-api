...
export const PHRASE_COLLISIONS = [
    {
        id: "jexxxus-docs-law",
        pattern: /\b(?:docs?\s+and\s+law|jexxxus\s+docs?\s+and\s+law|(?:what\s+(?:can you\s+)?tell me|tell me)\s+about\s+docs?\s+and\s+law)\b/i,
        tools: [],
        docsHint: true,
    ...
    {
        id: "jexxxus-docs",
        pattern: /\b(?:jexxxus\s*\|\s*docs?|docs\.jexxx\.us|(?:what\s+(?:can you\s+)?tell me|tell me)\s+about\s+(?:the\s+)?docs?|what(?:'s| is)\s+(?:jexxxus\s*\|\s*)?docs?)\b/i,
        tools: [],
        docsHint: true,
    ...
    {
        id: "jexxxus-law",
        pattern: /\b(?:jexxxus\s*\|\s*law|law\.jexxx\.us|(?:what\s+(?:can you\s+)?tell me|tell me)\s+about\s+(?:the\s+)?law|what(?:'s| is)\s+(?:jexxxus\s*\|\s*)?law|terms of service|privacy policy|refund policy|dmca policy)\b/i,
        tools: [],
        lawQuery: { action: "list" },
    ...
    {
        id: "tv-categories",
        pattern: /\b(in\s+church|pastor\/?priest|pastor'?s?\s+wife|nuns?|crucifixion|orgies|lesbo|bdsm|adultery|ai\s+sacraments|deepthroat|creampie|wedding)\b/i,
        tools: ["tv_query"],
    ...
    },
    {
        ...
        pattern: /\b(read the draft|unpublished (?:piece|article|post)|draft (?:on|article|piece)|number\s+\d+.*(?:article|piece|post))\b/i,
        tools: ["veil_query"],
        veilSearchQuery: "church girl",
    ...
    {
        id: "jexxxus-music",
        pattern: /\b(?:jexxxus\s*\|\s*music|music\.jexxx\.us|(?:what\s+(?:can you\s+)?tell me|tell me)\s+about\s+(?:the\s+)?music|what(?:'s| is)\s+(?:jexxxus\s*\|\s*)?music|crucifly\s*records?)\b/i,
        tools: ["music_query"],
        note: "JEXXXUS Music / Crucifly Records — music_query list or docs.",
    },
    {
        id: "music-surface",
        pattern: /\b(music\.jexxx\.us|jexxxus\s*\|\s*music|crucifly\s*records?|song\s+of\s+dylan|walk\s+on\s+wavs|traktrain|beat\s+store|sound\s+kits?|instrumentals?|lease\s+(?:a\s+)?beat|buy\s+beats?|gospel\s+rap|christian\s+hip[\s-]?hop)\b/i,
        tools: ["music_query"],
...
export function buildKingdomRoutingText(userPrompt, options) {
...
export function extractRoutingContextFromHistory(messages, maxMessages = 8) {
    return messages
        .slice(-maxMessages)
...
function addTools(set, tools) {
    for (const t of tools)
        set.add(t);
}
function removeTools(set, tools) {
    for (const t of tools)
        set.delete(t);
}
function mergeCompanionVerses(target, verses) {
    for (const v of verses) {
        if (target.size >= MAX_COMPANION_VERSES)
            break;
        target.add(v);
    }
}
/** Resolve tv_query search text from routing plan + prompt. */
export function inferTvSearchQuery(prompt, plan) {
    if (plan.tvSearchQuery)
        return plan.tvSearchQuery;
    if (/forgive\s*,?\s*me\s*,?\s*father/i.test(prompt))
        return "Forgive Me Father";
    if (/\b(mormon\s+girlz)\b/i.test(prompt))
        return "Mormon Girlz";
    if (/\bdeviante\b/i.test(prompt))
        return "Deviante";
    if (/\b(pastor\/?priest)\b/i.test(prompt))
        return "Pastor/Priest";
    if (/\bnuns?\b/i.test(prompt))
        return "Nuns";
    return null;
}
/** Infer fitting companions when TV/VEIL matched but no row supplied verses. */
export function inferThemeCompanionVerses(prompt) {
...
export function inferVeilSearchQuery(prompt, plan) {
    if (plan.veilSearchQuery)
        return plan.veilSearchQuery;
    if (/proverbs\s*31/i.test(prompt))
        return "Proverbs 31";
    if (/\b(ruth|boaz|naomi)\b/i.test(prompt))
        return "Ruth";
    if (/\b(rachels?|leahs?)\b/i.test(prompt))
        return "Rachel Leah";
    if (/\b(church\s+girl|churchy)\b/i.test(prompt))
        return "church girl";
    if (/\b(corruption|confession|altar)\b/i.test(prompt))
        return "corruption";
    if (/\bpastor'?s?\s+wife\b/i.test(prompt))
        return "pastor's wife";
    return null;
}
function isCatalogOnlyPrompt(prompt) {
    return (/\b(latest|recent|list)\b/i.test(prompt) &&
...
export function planKingdomTools(userPrompt, options) {
    const tools = new Set();
    const exclude = new Set();
    const slashHints = new Set();
    const companionVerses = new Set();
    let tvSearchQuery = null;
    let veilSearchQuery = null;
    let docsHint = false;
    let lawQuery = null;
    const matchedRules = [];
    const prompt = buildKingdomRoutingText(userPrompt, options);
    const hasVerseRef = looksLikeVerseReference(prompt) ||
    ...
    for (const row of PHRASE_COLLISIONS) {
        ...
        if (row.docsHint)
            docsHint = true;
        if (row.lawQuery && !lawQuery)
            lawQuery = row.lawQuery;
    }
    ...
    if (/\b(scripture|verse)\b/i.test(prompt) &&
    ...
    if (hasContentSurface && !isCatalogOnlyPrompt(prompt) && companionVerses.size === 0) {
        ...
        if (companionVerses.size > 0)
            matchedRules.push("theme-companions-inferred");
    }
    if (companionVerses.size > 0) {
        addTools(tools, ["bible_query"]);
...
export function detectNamedDivinities(text) {
    const names = [];
    if (/\bruth\b/i.test(text))
        names.push("Ruth");
    if (/\blil'?\s*bible\b/i.test(text))
        names.push("Lil' Bible");
    if (/\bhannah\b/i.test(text))
        names.push("Hannah");
    return names;
}
/** Human-readable block appended to the system prompt for the current user turn. */
export function formatKingdomRoutingHint(userPrompt, options) {
    ...
    if (plan.docsHint || plan.lawQuery) {
        lines.push("Do NOT call account_query — Docs and Law are public JEXXXUS surfaces, not BLXCKBOOK contacts.");
    }
    if (plan.docsHint) {
        lines.push("JEXXXUS | Docs (docs.jexxx.us) — summarize from Relevant JEXXXUS documentation context below (architecture, CLI, platform).");
    }
    if (plan.lawQuery) {
        const q = plan.lawQuery.query ? ` query="${plan.lawQuery.query}"` : "";
        lines.push(`Law (law.jexxx.us) — call law_query action=${plan.lawQuery.action}${q} for Terms, Privacy, Refunds, DMCA.`);
    }
    if (plan.tools.length > 0) {
    ...
    if (plan.tvSearchQuery && plan.tools.includes("tv_query")) {
        lines.push(`TV search — call tv_query action=search query="${plan.tvSearchQuery}" (not action=list).`);
    }
    if (plan.veilSearchQuery && plan.tools.includes("veil_query")) {
        lines.push(`VEIL search — call veil_query action=search query="${plan.veilSearchQuery}" (not action=list).`);
    }
    if (plan.tools.includes("music_query")) {
        lines.push("Music (music.jexxx.us) — call music_query action=list or action=docs for Crucifly Records, beats, kits, and Traktrain links. Never invent track titles.");
    }
    if (options?.conversationContext?.trim()) {
        lines.push("Routing includes recent conversation context — use tools even during persona roleplay when scripture, drafts, or VEIL themes appear.");
    }
    const personas = detectNamedDivinities(buildKingdomRoutingText(userPrompt, options));
    if (personas.length >= 2) {
        lines.push(`Persona cast — labeled sections for each: ${personas.join(", ")}. Include every named character with equal depth.`);
    }
    if (plan.companionVerses.length > 0) {
        lines.push(`Companion scripture — call bible_query action=query separately for each: ${plan.companionVerses.join("; ")}`);
        lines.push("Quote 2–3 of these verses in your final reply alongside TV/VEIL links. " +
    ...
    if (plan.companionVerses.length > 0 || plan.tvSearchQuery) {
        lines.push("Pre-fetched TV/scripture may appear below — synthesize a polished answer from that data.");
    }
    lines.push("Synthesize TV + VEIL + scripture in one answer. Do not retry a failed bible_query with format variants.");
    return lines.join("\n");
}
...
//# sourceMappingURL=kingdom-routing.js.map