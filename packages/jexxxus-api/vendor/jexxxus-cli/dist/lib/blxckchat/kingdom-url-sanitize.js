...
export function splitGluedKingdomUrls(text) {
    let out = text;
    out = out.replace(/(https?:\/\/(?:veil\.jexxx\.us\/articles|tv\.jexxx\.us\/video)\/[a-z0-9-]+)(?=https?:\/\/)/gi, "$1\n");
    out = out.replace(/([a-z0-9-])(https?:\/\/(?:veil\.jexxx\.us|tv\.jexxx\.us)\/)/gi, "$1\n$2");
    return out;
}
function extractSlug(url, surface) {
    const prefix = surface === "tv"
...
export function extractKingdomUrlsFromText(text) {
    const seen = new Set();
    const entries = [];
    const add = (surface, url, slug, title) => {
        const key = `${surface}:${slug}`;
        if (seen.has(key))
            return;
        seen.add(key);
        const entry = { surface, url, slug };
        if (title)
            entry.title = title;
        entries.push(entry);
    };
 ...
 */
function closestCatalogSlug(compact, catalog, surface) {
...
function canonicalizeUrl(raw, catalog) {
...
export function compactKingdomBulletLinks(text) {
    return text.replace(/^(\s*[•*-]\s+)(.+?)\s*\[(https?:\/\/(?:veil|tv)\.jexxx\.us\/[^\]\n]+)\]\s*$/gm, (_full, bullet, title, url) => `${bullet}[${title.trim()}](${url.replace(/\s+/g, "")})`);
}
/** Turn `[url1\nurl2]` blobs into one bullet per URL. */
export function repairMarkdownUrlBlobs(text) {
 ...
 */
export function sanitizeKingdomUrls(response, catalog) {
    if (!response.trim())
        return response;
    ...
    out = out.replace(/https?:\/\/(?:tv|wv|tw)\.jexxx\.us\/video\/([a-z0-9][a-z0-9\s-]*[a-z0-9]|[a-z0-9])(?=[\s\]\),.;!?]|$)/gi, (_full, slugPart) => {
    ...
    out = out.replace(KINGDOM_TV_URL, (url) => canonicalizeUrl(url.replace(/\s+/g, ""), catalog) ?? UNVERIFIED_LINK);
    out = out.replace(/https?:\/\/veil\.jexxx\.us\/articles\/([a-z0-9][a-z0-9\s-]*[a-z0-9]|[a-z0-9])(?=[\s\]\),.;!?]|$)/gi, (_full, slugPart) => {
    ...
    out = out.replace(KINGDOM_VEIL_URL, (url) => canonicalizeUrl(url.replace(/\s+/g, ""), catalog) ?? UNVERIFIED_LINK);
    out = repairMarkdownUrlBlobs(out);
    out = compactKingdomBulletLinks(out);
    return out;
}
//# sourceMappingURL=kingdom-url-sanitize.js.map