...
import { extractContactDeleteFromText, isContactDeletePrompt, } from "./account-routing.js";
/**
 ...
 */
export async function executeDeterministicContactDeleteIfRequested(userPrompt, session) {
    ...
    try {
        ...
        verifyText = `account_query verification failed: ${err instanceof Error ? err.message : String(err)}`;
    }
    ...
    if (verified) {
        ...
        lines.push("", `WARNING: delete reported success but "${contactName}" still appears in account_query — tell the user honestly.`);
    }
    else if (!deleteResult.ok) {
        lines.push("", "DELETE FAILED — tell the user honestly; do not claim the contact was removed.");
    }
    lines.push("", "Do NOT cite earlier conversation turns for who remains in the vault — only the account_query output above.", "Do NOT call delete_contact again for the same name unless the user repeats the request.");
    return { executed: true, text: lines.join("\n") };
}
//# sourceMappingURL=vault-deterministic-write.js.map