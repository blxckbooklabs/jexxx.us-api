...
export function formatToolResultForFallback(toolName, raw) {
    ...
    if (toolName === "bible_query") {
        if (trimmed.startsWith("Chapter list for") || trimmed.includes("navigation only")) {
            return ("I need a full verse reference (e.g. 1 John 1:9) rather than a chapter list. " +
        ...
        if (looksLikeChapterJsonArray(trimmed)) {
            return ("I retrieved a chapter list instead of verse text. " +
    ...
    if (looksLikeChapterJsonArray(trimmed)) {
        return ("I retrieved catalog metadata but not the content you asked for. " +
            "Please try again — I will search TV/VEIL and quote companion scripture.");
    }
    return trimmed;
}
function looksLikeChapterJsonArray(trimmed) {
...
//# sourceMappingURL=tool-result-format.js.map