 ...
 * TV_PUBLIC_BASE_URL.
 */
export function assertAllowedTvPublicBaseUrl(rawUrl) {
    let parsed;
    try {
    ...
    if (!ALLOWED_PUBLIC_HOSTS.has(parsed.hostname)) {
        throw new Error(`[TV] Public base URL host not allowed: ${parsed.hostname}. ` +
            "Only tv.jexxx.us (or localhost for dev) is permitted.");
    }
    return rawUrl.replace(/\/$/, "");
}
/** Read a single approved JSON catalog file — no traversal. */
export function readPublicJsonCatalog(filePath) {
...
export function readPublicLlmsFile(publicDir, filename) {
...
//# sourceMappingURL=tv-security.js.map