...
export function formatCredentialsDisplayName(creds) {
...
export function formatCredentialsShortLabel(creds) {
    const name = creds.fullName?.trim();
    if (name)
        return name;
 ...
 */
export async function buildOperatorIdentityContext(session) {
    const { creds } = session;
    ...
    if (session.isSuperAdmin) {
        if (session.operator) {
            accessLines.push("Super-admin elevation: ACTIVE — service-role operator clients are available for cross-user kingdom queries when account_query includes asUserId (another Clerk user ID). Personal vault questions still default to this operator's own RLS scope unless asUserId is set.");
        }
        else {
            accessLines.push("Super-admin elevation: recognized, but SUPABASE_KEY (service role) is not loaded in jexxx.us-cli/.env — only RLS-scoped reads are available in this session.");
        }
    }
    try {
...
export function buildOfflineOperatorIdentityContext(creds) {
