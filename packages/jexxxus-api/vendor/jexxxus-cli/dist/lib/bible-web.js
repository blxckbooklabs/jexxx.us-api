...
async function fetchChapterVerses(url) {
    try {
        ...
        if (data.success === false)
            return null;
        return ((data.success && data.data?.verses) ||
            data.data?.verses ||
            data.verses ||
            null);
    }
...
async function fetchVerseFromBibleApiCom(slug, bookName, chapter, verse, translation) {
    let transParam = "web";
