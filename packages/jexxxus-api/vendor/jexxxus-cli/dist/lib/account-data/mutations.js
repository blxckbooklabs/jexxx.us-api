...
function sanitizeContactUpdates(updates) {
    const fields = {};
    const rejected = [];
    for (const [key, value] of Object.entries(updates)) {
 ...
 */
export async function addContact(session, name, options) {
    ...
    const existing = fuzzyMatchContact((existingRows ?? []), name);
    if (existing) {
        return {
            ok: false,
            message: `A contact matching "${name}" already exists ("${existing.name}") — ` +
...
export async function updateContact(session, target, contactName, updates) {
    ...
    const match = fuzzyMatchContact((rows ?? []), contactName);
    if (!match) {
 ...
 */
export async function addJournalEntry(session, title, content, options) {
    const vault = resolveVaultClient(session, "blxckbook");
    ...
    if (options?.linkedContactNames?.length) {
        ...
        const linkRows = options.linkedContactNames
            .map((name) => fuzzyMatchContact((contacts ?? []), name))
            .filter((c) => Boolean(c))
            .map((c) => ({
            user_id: vault.effectiveUserId,
            journal_id: entryId,
...
async function findJournalEntry(vault, entryIdOrTitle) {
    ...
    return (rows.find((r) => normalizeName(r.title ?? "") === needle) ??
...
export async function updateJournalEntry(session, entryIdOrTitle, updates) {
    const vault = resolveVaultClient(session, "blxckbook");
...
export async function deleteJournalEntry(session, entryIdOrTitle) {
    const vault = resolveVaultClient(session, "blxckbook");
    const entry = await findJournalEntry(vault, entryIdOrTitle);
    if (!entry) {
        return { ok: false, message: `No journal entry matching "${entryIdOrTitle}" found.` };
...
export async function deleteContact(session, target, contactName) {
    let vault;
    try {
        vault = resolveVaultClient(session, target);
    }
 ...
 */
export async function addContactEvent(session, contactName, fields) {
    const vault = resolveVaultClient(session, "nxt");
    const { data: vessels } = await vault.client
        .from("vessels")
        .select("*")
        .eq("user_id", vault.effectiveUserId);
    const vessel = fuzzyMatchContact((vessels ?? []), contactName);
    if (!vessel) {
    ...
    if (fields.eventType !== undefined)
        row.event_type = fields.eventType;
...
export async function updateContactEvent(session, eventId, updates) {
    const vault = resolveVaultClient(session, "nxt");
    const fields = {};
    if (updates.title !== undefined)
        fields.title = updates.title;
    if (updates.eventDate !== undefined)
        fields.event_date = updates.eventDate;
    if (updates.eventType !== undefined)
        fields.event_type = updates.eventType;
...
export async function deleteContactEvent(session, eventId) {
    const vault = resolveVaultClient(session, "nxt");
    const { error } = await vault.client
        .from("contact_events")
        .delete()
...
export async function managePlaylist(session, action, playlistName, options) {
    ...
    switch (action) {
        case "create": {
            ...
            if (error)
                return { ok: false, message: `Failed to create playlist: ${error.message}` };
        ...
        case "rename": {
            ...
            if (!playlist)
                return { ok: false, message: `No playlist matching "${playlistName}" found.` };
        ...
        case "add_video": {
            ...
            if (error)
                return { ok: false, message: `Failed to add video: ${error.message}` };
        ...
        case "remove_video": {
            ...
            if (error)
                return { ok: false, message: `Failed to remove video: ${error.message}` };
 ...
 */
export async function syncBlxckbookExport(session, payload) {
    const vault = resolveVaultClient(session, "blxckbook");
    const summary = [];
