...
export function getJexxxusApiBaseUrl() {
    ...
    if (flag && DISABLED.has(flag))
        return null;
    const url = process.env.JEXXXUS_API_URL?.trim() ||
        process.env.JEXXXUS_ACCOUNT_API_URL?.trim() ||
        "https://api.jexxx.us";
    if (DISABLED.has(url.toLowerCase()))
        return null;
    return url.replace(/\/$/, "");
}
async function resolveAccessToken(session) {
...
async function accountApiFetch(session, path, init) {
    ...
    try {
        ...
        throw new Error(`Cannot reach JEXXXUS | API at ${base} (${detail}). ` +
            "Start the Mac API (port 8787) or set JEXXXUS_ACCOUNT_API=off for direct Supabase.");
    }
    const text = await response.text();
    if (!response.ok) {
        throw new Error(`JEXXXUS | API ${response.status}${text ? `: ${text.slice(0, 240)}` : ""}`);
    }
    if (!text.trim()) {
...
export async function fetchAccountSummaryViaApi(session, asUserId) {
    const qs = asUserId ? `?asUserId=${encodeURIComponent(asUserId)}` : "";
    const payload = await accountApiFetch(session, `/api/v1/account/summary${qs}`);
    if (!payload?.data) {
        throw new Error("JEXXXUS | API summary response missing data.");
    }
    return payload.data;
}
export async function executeAccountQueryViaApi(session, args) {
    const payload = await accountApiFetch(session, "/api/v1/account/query", {
        method: "POST",
