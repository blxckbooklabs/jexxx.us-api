...
function getDaysSinceUpload(dateString) {
    if (!dateString || dateString === "Just now")
        return 0;
...
export function calculateDevotionScore(video) {
    ...
    let baseScore = views * viewWeight + likes * likeWeight + shares * shareWeight + saves * saveWeight;
 ...
 */
export function heavyRankerShuffle(videos) {
    if (videos.length === 0)
        return [];
...
export function recommendTvVideos(videos, limit) {
    return heavyRankerShuffle(videos).slice(0, limit);
}
//# sourceMappingURL=tv-algorithm.js.map