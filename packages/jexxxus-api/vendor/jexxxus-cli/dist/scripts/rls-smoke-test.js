...
async function main() {
    ...
    if (!tokenA || !tokenB || !supabaseUrl || !supabaseAnonKey) {
        fail("Missing required env vars. Need JEXXXUS_RLS_TEST_TOKEN_A, " +
            "JEXXXUS_RLS_TEST_TOKEN_B, SUPABASE_URL, SUPABASE_ANON_KEY. See the " +
            "usage comment at the top of this file.");
    }
    const env = { supabaseUrl: supabaseUrl, supabaseAnonKey: supabaseAnonKey };
    ...
    if (errorA)
        fail(`User A query failed: ${errorA.message}`);
    if (!rowsA || rowsA.length === 0) {
        fail("User A has zero contacts — seed at least one contact for this account before running.");
    }
    const userIdA = rowsA[0].user_id;
    const leakedIntoA = rowsA.filter((r) => r.user_id !== userIdA);
    if (leakedIntoA.length > 0) {
        fail(`User A's query returned ${leakedIntoA.length} row(s) belonging to a different user_id — RLS is not enforcing per-user scoping!`);
    }
    ...
    if (errorB)
        fail(`User B query failed: ${errorB.message}`);
    if (!rowsB || rowsB.length === 0) {
        fail("User B has zero contacts — seed at least one contact for this account before running.");
    }
    const userIdB = rowsB[0].user_id;
    const leakedIntoB = rowsB.filter((r) => r.user_id !== userIdB);
    if (leakedIntoB.length > 0) {
        fail(`User B's query returned ${leakedIntoB.length} row(s) belonging to a different user_id — RLS is not enforcing per-user scoping!`);
    }
    ...
    const crossUserRows = rowsA.filter((r) => r.user_id === userIdB);
    if (crossUserRows.length > 0) {
        fail(`CRITICAL: User A's unfiltered query returned ${crossUserRows.length} row(s) belonging to User B. RLS policy is not enforcing per-user access on api.contacts.`);
    }
...
//# sourceMappingURL=rls-smoke-test.js.map