...
function resolveVoice(voice: string): string {
  ...
  if (ALLOWED_VOICE_IDS.has(trimmed)) return trimmed;
  if (/^[a-z]{2}-[A-Z]{2}-[A-Za-z0-9]+Neural$/u.test(trimmed)) return trimmed;
...
function cacheKey(voice: string, rate: string, text: string): string {
...
export const ttsRoutes = async (fastify: FastifyInstance) => {
  ...
  fastify.post("/", async (request: any, reply: any) => {
    try {
      ...
      fastify.log.info(
        `[TTS] edge voice=${voice} rate=${rate} chars=${text.length}`,
      );
