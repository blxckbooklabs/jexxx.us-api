...
const findSovereignFiles = (bookSlug: string, chapter: number) => {
...
export const bibleRoutes = async (server: FastifyInstance) => {
            ...
            try {
                ...
                return {
                    ...
                    data: {
                        ...
                        translation: (data as any).translation_name,
                        chapters: (data as any).chapter,
                        verses: (data as any).verses?.map((v: any) => ({
                            verse: v.verse,
                            text: v.text.replace(/\n/g, ' ').trim()
                        })) || []
                    }
    ...
    server.get(
        ...
        async (request: any, reply: any) => {
            ...
            const translation = (request.query as any).translation || 'KJV';
            ...
            try {
                const response = await fetch(
                    `${BIBLE_API_BASE}/${encodeURIComponent(book)}+${chapter}:${verse}?translation=${translation}`
                );
                ...
                return {
                    ...
                    data: {
                        reference: (data as any).reference,
                        translation: (data as any).translation_name,
                        book: (data as any).book_name,
                        chapter: (data as any).chapter,
                        verse: (data as any).verse,
                        text: (data as any).text.replace(/\n/g, ' ').trim()
                    }
                };
            } catch (error) {
                return (reply as any).status(500).send({
