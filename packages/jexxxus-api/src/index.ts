...
const loadServer = async () => {
  ...
  if (!startup.ok) {
    console.warn(
      `[JEXXXUS | API] Vault config incomplete — missing: ${startup.missing.join(", ")}. ` +
  ...
  server.register(async (protectedApp: any) => {
    protectedApp.addHook("onRequest", authMiddleware as any);
    protectedApp.addHook("onRequest", tracingMiddleware as any);
    protectedApp.addHook("onResponse", tracingResponseHook as any);

    protectedApp.register(accountRoutes, { prefix: "/api/v1/account" });

    protectedApp.register(async (toolsApp: any) => {
    ...
    if (observabilityRoutesEnabled()) {
      protectedApp.register(async (obsApp: any) => {
        obsApp.addHook("onRequest", requireSuperAdmin as any);
  ...
  console.log(
    `JEXXXUS | API listening on :${port} (surface=${surface}, vault=${startup.ok ? "ready" : "misconfigured"})`,
  );
};
